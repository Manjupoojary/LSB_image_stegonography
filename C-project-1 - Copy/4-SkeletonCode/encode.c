...
Status do_encoding(EncodeInfo *encInfo)
{
    ...
    if(open_files(encInfo)==e_success)
    {
        ...
        if(check_capacity(encInfo)==e_success)
        {
                         ...
                         if(encode_secret_file_size(encInfo->image_capacity,encInfo)==e_success)
                         {
                             ...
                             if(encode_secret_file_data(encInfo)==e_success)
                             {
                                 printf("Encoded secret file data successfully\n");
                                     ...
                                     {
                                         printf("Copying of remaining image data was unsuccessful\n");
                                         return e_failure;
                                     }
                             ...
                             {
                                 printf("Encoding of secret file data was unsuccessfull\n");
                                 return e_failure;
                             }
                         }
                         else
                         {
                             printf("Encoding file size was failure\n");
                             return e_failure;
                         }
                      }
                      else
                         {
                             printf("Encoded extn was a failure\n");
                             return e_failure;
                         }
                    }
                    else
                    {
                        printf("Ecoding secret file extn size was a failure\n");
                        return e_failure;
                    }
                }
                else
            ...
            {
                printf("couldnot copy bmp header\n");
                return e_failure;
            }
        }
        else
